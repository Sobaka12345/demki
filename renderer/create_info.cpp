...
{
    for (auto& [bindingId, resource] : m_resources)
    {
        resource->bind(context, bindingId);
    }
};

void IPipeline::Descriptor::setBinding(uint32_t bindingId,
    std::shared_ptr<IShaderResource> resource)
{
    m_resources[bindingId] = resource;
}

std::shared_ptr<IShaderResource> IPipeline::Descriptor::binding(uint32_t bindingId)
{
    return m_resources[bindingId];
};

// void IPipeline::Object::setBinding(uint32_t bindingId, std::shared_ptr<IShaderResource> resource)
// {
//     m_descriptor->setBinding(bindingId, resource);
// }

// void IPipeline::Object::bind(OperationContext& context)
// {
//     return m_descriptor->bind(context);
// }
...
//  {
//      pixels = stbi_load(path.string().c_str(), &width, &height, &textureChannels,
//      STBI_rgb_alpha); imageSize = width * height * 4; mipLevels =
...
//  {
//      pixels = stbi_load(path.string().c_str(), &width, &height, &textureChannels,
//      STBI_rgb_alpha); imageSize = width * height * 4; mipLevels = mipLevels;
//  }
