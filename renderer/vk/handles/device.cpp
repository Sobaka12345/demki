...
{
    return std::find(m_queueFamilyIndices.begin(), m_queueFamilyIndices.end(), s_invalidIndex) ==
...
std::weak_ptr<Queue> Device::queue(QueueFamilyType type, uint32_t idx) const
{
    ...
    if (it == m_queues.end())
    {
        auto [res, _] = m_queues.emplace(std::pair{
            std::pair{ familyIdx, idx }, std::make_shared<Queue>(*this, familyIdx, idx) });
        return res->second;
    }
