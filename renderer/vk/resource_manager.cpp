	...
	std::span<const Vertex3DColoredTextured> vertices, std::span<const uint32_t> indices)
{
    ...
    m_modelBuffers.emplace_back(m_device, BufferCreateInfo()
        .size(verticesSize)
    ...
    m_indBuffers.emplace_back(m_device, BufferCreateInfo()
        .size(indicesSize)
        .usage(VK_BUFFER_USAGE_TRANSFER_DST_BIT |
    ...
    Buffer stagingBuffer(m_device, Buffer::staging()
        .size(indicesSize + verticesSize));
    auto mapped = stagingBuffer.allocateAndBindMemory(
    ...
    stagingBuffer.copyTo(m_modelBuffers.back(), create::bufferCopy(verticesSize));
    stagingBuffer.copyTo(m_indBuffers.back(), create::bufferCopy(indicesSize, verticesSize));

    return std::make_shared<Model>(Model::Descriptor{
        .vertices = Buffer::Descriptor{
            .buffer = m_modelBuffers.back(),
            .offset = 0,
            .range = verticesSize,
            .objectSize = vertexSize
        },
        .indices = Buffer::Descriptor{
...
std::shared_ptr<Model> ResourceManager::loadModel(std::filesystem::path path, Resource::Storage targetStorage)
{
