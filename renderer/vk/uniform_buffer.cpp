    ...
    , m_createInfo(std::move(createInfo))
{
    ...
    m_buffer = BufferHelper::create(context.device(), &bufferCreateInfo, nullptr);
    ...
    const auto memAllocInfo = MemoryAllocateInfo{}
                        .allocationSize(memRequirements.size)
                        .memoryTypeIndex(DeviceMemoryHelper::findMemoryType(context.physicalDevice(),
                            memRequirements.memoryTypeBits, properties));
    m_memory = DeviceMemoryHelper::create(context.device(), &memAllocInfo, nullptr);
...
{
    DeviceMemoryHelper::destroy(m_context.device(), m_memory, nullptr);
    BufferHelper::destroy(m_context.device(), m_buffer, nullptr);
}
